...
function emojify(t, color) {
	return t;
	for (em in emotes) {
		t = t.replaceAll(":" + emotes[em] + ":", `</span><span class="tooltip" data-tooltip=":${emotes[em]}:" style="--color: ${color}; color: ${color};"><img src="../images/emote/${emotes[em]}.png" style="height: 20px; width: 20px;"/></span><span class="message-inner">`);
	}
	for (em in icons) {
		t = t.replaceAll(':' + icons[em] + ":", `</span><span class="tooltip" data-tooltip=":${icons[em]}:" style="--color: ${color}; color: ${color}; position: absolute;"><i class="fa-solid fa-${icons[em]}" style="font-size: 20px; margin: 0px; height: 0;"></i></span><span>&nbsp;&nbsp;&nbsp;</span><span class="message-inner">`)
	}
	return t;
}
